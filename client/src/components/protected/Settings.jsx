...
class Settings extends Component {
      ...
      .catch(err => {
		...
		handleSubmit = (event) => {
				...
				axios.post(`/users/${this.props.id}`,form)
				.then(() => {
				...
				let conditionalDisplay = !this.state.isTutor ? <div></div> : (
						...
						<FormGroup>
								{this.state.tests.map((test,i)=>{
																				return(
																						<Checkbox onChange={this.handleCheck} inline key={i} value={Number(test.id)} checked={(this.isPreselectedTests(test.id)? "checked" : undefined)}>{test.Name}</Checkbox>
																				)
																		})}
				...
				return (
						<div className= 'settings' >
								...
								<div className="image-settings">
									<img className="img-circle" src={this.state.photo} alt={`${this.state.name}'s profile picture`} />
								</div>
