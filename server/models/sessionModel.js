...
exports.getSession = (form, callback) => {
  if (form.isTutor) {
    let queryStr = `SELECT * FROM users RIGHT JOIN sessions ON users.id = sessions.student_id WHERE tutor_id = ${
      form.id
    } AND complete = 0 ORDER BY date ASC, time ASC`;
    db.query(queryStr, callback);
  } else {
    let queryStr = `SELECT * FROM tutors RIGHT JOIN sessions ON tutors.id = sessions.tutor_id WHERE student_id = ${
      form.id
    } AND complete = 0 ORDER BY date ASC, time ASC`;
    db.query(queryStr, callback);
  }
